...
> **当前默认模式：** 应用启动后会直接加载 Blockchain.info 公开接口中的最新未确认交易，无需输入地址。图中的有效 BTC 地址节点和交易详情端点均可直接跳转至对应的 Blockchain.com Explorer 页面；未知脚本与聚合节点不会创建跳转链接。
...
| 自动最新交易流 | 无需任何输入，直接以最新内存池交易生成资金流视图。 |
...
默认首页现在以最新内存池交易为数据源。蓝色输入节点及绿色输出节点中的有效 BTC 地址可点击并在新标签页打开对应的 Blockchain.com Explorer 地址页；琥珀色节点用于选择交易详情。若端点没有可公开解析的地址，应用会保留为只读标签，避免生成无效跳转。
